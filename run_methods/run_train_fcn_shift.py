...
import subprocess
import os
from rope_learn.utils import construct_run_command, construct_variants
...
def worker(gpu_id, max_per_gpu, exps):
    ...
    processes = []
    for exp in exps:
        exp['name'] = 'fcn_shift' + exp['name']
        args = construct_run_command('rope_learn/train_methods/train_fcn_shift.py', exp)
        print('Running', args)
...
if __name__ == '__main__':
    ...
    folders = ['data/rope_robot']
    exps = [dict(root=folders),
            dict(data_subset=[args.data_subset]),
            dict(batch_size=[128], lr=[1e-3], epochs=[500], learn_delta=[False]),
            dict(seed=[args.seed])]
