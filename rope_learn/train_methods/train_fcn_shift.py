...
def test(trans, test_loader, epoch, device):
    ...
    viz = trans.viz(obs, obs_pos, actionsi, obs_pos_h, actions)
    cv2.imshow('viz',viz)
    cv2.waitKey(1)
    wandb.log({"viz": [wandb.Image((viz*255).astype(np.uint8), caption="viz_{}".format(epoch))]})
...
def main():
    ...
    save_args = vars(args)
    save_args['script'] = 'train_fcn_pick_place'
    ...
    trans = cm.FCNShift(obs_dim, action_dim, learn_delta=args.learn_delta).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='fcn_pickplace')
